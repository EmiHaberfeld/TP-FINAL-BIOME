...
# Graficamente:gp_semana <- ggplot(prob_exito_semana_long, aes(x=tiempo_testeo, y=proporcion_exitos, colour=SEMANA,group=SEMANA)) +
  geom_line(linetype=2) +
  geom_point(size=2) +
...
```
*Figura 6:* proporciones estimadas de PER para cada tratamiento en cada tiempo de evaluación. Los resultados se informan como la media +/- el error estándar de cada combinación de tiempo y tratamiento. Se grafican los errores estándar y no los intervalos de confianza porque se superpondrían quitándole claridad al gráfico.
