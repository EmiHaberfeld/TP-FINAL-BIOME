...
# Simetría compuesta (m6): decimos que para cada abeja hay una misma correlacion entre tiempos. Elegimos este modelo.
...
$\frac{\pi_{ijk}}{(1-\pi_{ijk})}$= $e^{(\mu + \alpha_i + \beta_j + \alpha*\beta_{ij} + \gamma_k + A_{l(ij)(k)})}$
...
Con *i* = 1 *a* 4; *j* = 1 *a* 3;  *k* = 1 *a* 7 y *l* = 1 *a* ?
